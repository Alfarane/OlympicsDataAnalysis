...
def fetch_medal_tally(df ,year ,country):
    ...
    if flag == 1:
        x = temp_df.groupby('Year').sum()[['Gold' ,'Silver' ,'Bronze']].sort_values('Year' ,ascending=True)
    else:
...
def most_succesful(df, sport):
    ...
    medal_counts.columns = ['Name', 'count']  # Rename columns to 'Name' and 'count'
    return medal_counts.head(10).merge(df, on='Name', how='left')[['Name', 'count', 'Sport', 'region']].drop_duplicates()

def yearwise_medal_tally(df,country):

    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'City', 'Sport', 'Event', 'Medal', 'Year'], inplace=True)
...
def most_successful_countrywise(df,country):
    ...
    medal_counts = temp_df['Name'].value_counts().reset_index()
    medal_counts.columns = ['Name', 'Medals']  # Rename columns to 'Name' and 'count'
    return medal_counts.head(10).merge(df, on='Name', how='left')[['Name', 'Medals', 'Sport']].drop_duplicates('Name')

def height_weight_plot(df,sport):
